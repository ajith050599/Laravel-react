...
const AvgDwellTime = ({ config, isDraggable }) => {
  const location = config?.location || "Decathlon Deira";
  const timeframe = config?.time_frame || "today";

  const { data, isLoading, hasError } = useFetch(
    `/average-zone-dwell-time?location=${location}`,
    {},
    15,
    !isDraggable,
    false
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          <p className="text-gray-700 font-semibold text-sm tracking-wide">
            {config?.people_type?.toUpperCase() || "VISITOR"}
          </p>
      ...
      <div className="mt-6">
        ...
        {data && !isDraggable && (
          <p className="text-5xl font-semibold text-gray-800">
            {dwellTime} <span className="text-2xl font-normal">min</span>
          </p>
        )}
        {isDraggable && (
